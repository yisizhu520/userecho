...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('tenant_id', sa.String(36), sa.ForeignKey('tenants.id', ondelete='CASCADE'), 
                  nullable=False, index=True, comment='租户ID'),
        sa.Column('config_group', sa.String(32), nullable=False, 
                  comment='配置分组（clustering/notification/display）'),
        sa.Column('config_data', sa.JSON() if bind.dialect.name == 'postgresql' else sa.Text(), 
                  nullable=False, comment='配置数据（JSON格式）'),
