...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('used_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.text('now()'), comment='使用时间'),
        sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True, comment='更新时间'),
    ...
    op.create_table(
        ...
        sa.Column('type', sa.String(20), nullable=False, server_default='disposable', comment='类型: disposable/spam/banned'),
    ...
    op.create_index('idx_email_blacklist_active', 'email_blacklist', ['is_active'], 
    ...
    op.create_table(
        ...
        sa.Column('last_sent_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.text('now()'), comment='上次发送时间'),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.text('now()'), comment='创建时间'),
        sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True, comment='更新时间'),
        sa.ForeignKeyConstraint(['user_id'], ['sys_user.id'], ondelete='CASCADE'),
    )
