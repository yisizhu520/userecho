...
class CRUDTenantMember:
    ...
    async def get_list(db: AsyncSession, tenant_id: str, *, status: str | None = None) -> Sequence[TenantUser]:
        """获取租户的成员列表"""
        stmt = select(TenantUser).where(TenantUser.tenant_id == tenant_id).order_by(TenantUser.joined_at.desc())
        if status:
    ...
    @staticmethod
    async def update(db: AsyncSession, member: TenantUser, *, user_type: str | None = None, status: str | None = None) -> TenantUser:
        """更新成员"""
    ...
    @staticmethod
    async def set_member_roles(db: AsyncSession, tenant_user_id: str, role_ids: list[str], assigned_by: int | None = None) -> None:
        """设置成员的角色（全量覆盖）"""
        # 删除旧的角色关联
        stmt = select(TenantUserRole).where(TenantUserRole.tenant_user_id == tenant_user_id)
        result = await db.execute(stmt)
