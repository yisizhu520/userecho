...
class PriorityService:
    ...
    async def suggest_impact_scope_fast(
        ...
        if customer_count >= 10:
            return {
                'scope': 10,
                'confidence': 0.7,
                'reason': f'影响 {customer_count} 个客户，判定为全部用户'
            }
        elif customer_count >= 5:
            return {
                'scope': 5,
                'confidence': 0.7,
                'reason': f'影响 {customer_count} 个客户，判定为大多数用户'
            }
        elif customer_count >= 2:
            return {
                'scope': 3,
                'confidence': 0.6,
                'reason': f'影响 {customer_count} 个客户，判定为部分用户'
            }
        else:
            ...
            if feedback_count >= 5:
                return {
                    'scope': 3,
                    'confidence': 0.5,
                    'reason': f'1 个客户提交 {feedback_count} 条反馈，可能影响范围较大'
                }
            return {
                'scope': 1,
                'confidence': 0.6,
                'reason': '仅影响 1 个客户，判定为个别用户'
            }
    ...
    async def suggest_dev_cost_fast(
        ...
        return {
            'days': days,
            'confidence': 0.5,
            'reason': f'基于分类 {category} 的经验值：{reason}'
        }
