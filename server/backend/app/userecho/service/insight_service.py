...
class InsightService:
    ...
    async def _generate_priority_suggestions(
        ...
        scored_topics.sort(key=lambda x: (
            self.URGENCY_PRIORITY[x['urgency_level']],
            -x['estimated_roi']
        ))
    ...
    async def _classify_urgency(self, db: AsyncSession, topic: Topic) -> str:
        ...
        has_major_customer = any(
            c.customer_type in ['major', 'strategic'] for c in customers
        )
...
"""
    ...
    async def _collect_report_data(
        ...
        query_last_feedbacks = select(func.count(Feedback.id)).where(
            and_(
                Feedback.tenant_id == tenant_id,
                Feedback.submitted_at.between(last_start, last_end)
            )
        )
        ...
        query_top = (
            ...
            .order_by(
                Topic.feedback_count.desc(),
                PriorityScore.total_score.desc()
            )
            .limit(3)
        )
        ...
        for topic in top_topics_raw:
            # 获取客户数
            query_customers = select(func.count(func.distinct(Customer.id))).join(
                Feedback, Customer.id == Feedback.customer_id
...
"""
    ...
    async def _calculate_sentiment_trend(
        ...
        negative_topics = [
            {'topic_id': row[0], 'title': row[1], 'negative_count': row[2]}
            for row in result_negative.all()
        ]
        ...
        if change > 10:
            ...
            summary = f'⚠️ 本周正面反馈占比 {this_positive_rate:.0f}%，较上周下降 {abs(change):.0f}%，需要关注客户满意度问题。'
        elif change < 0:
