...
class ModelEntry(ScheduleEntry):
    ...
    def __init__(self, model: TaskScheduler, app=None) -> None:
        super().__init__(
    ...
    async def to_model_schedule(name: str, task: str, schedule: schedules.schedule | TzAwareCrontab) -> TaskScheduler:
        ...
        async with async_db_session() as db:
            if isinstance(schedule, schedules.schedule):
                ...
                crontab = f"{schedule._orig_minute} {schedule._orig_hour} {schedule._orig_day_of_week} {schedule._orig_day_of_month} {schedule._orig_month_of_year}"
...
@beat_init.connect
def acquire_distributed_beat_lock(sender=None, **kwargs) -> None:
    """
    尝试在启动时获取锁
