...
async def measure_redis_latency() -> dict:
    ...
    try:
        ...
        if elapsed > 100:
            ...
            print(f"   这通常意味着你的 Redis 在远程服务器上")
            print(f"   每次操作都有这个网络延迟开销")
        ...
        for i in range(10):
            ...
            # GET
            start = time.perf_counter()
            value = await redis.get(f'test:diagnose:{i}')
        ...
        try:
            ...
            async with async_db_session() as session:
                ...
                result = await session.execute(
                    text("SELECT COUNT(*) FROM sys_user LIMIT 1")
                )
                count = result.scalar()
