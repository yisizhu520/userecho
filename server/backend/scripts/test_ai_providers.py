...
class SimpleAIClient:
    ...
    async def test_embedding(self, text: str):
        ...
        try:
            ...
            response = await self.clients[self.current_provider].embeddings.create(model=embedding_model, input=text)
    ...
    async def test_chat(self, prompt: str):
        ...
        try:
            ...
            response = await self.clients[self.current_provider].chat.completions.create(
                model=chat_model, messages=[{'role': 'user', 'content': prompt}], max_tokens=100, temperature=0.7
            )
