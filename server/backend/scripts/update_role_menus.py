...
async def update_role_menus() -> None:
    """更新角色菜单权限"""
    async with async_db_session.begin() as db:
        ...
        for role_name in full_access_roles:
            ...
            # 清除现有权限
            await db.execute(delete(role_menu).where(role_menu.c.role_id == role.id))
        ...
        for role_name, menu_paths in specific_access.items():
            ...
            if settings_menu and role_name in ["PM", "老板"]:  # 只给 PM 和老板分配设置权限
                await db.execute(role_menu.insert().values(role_id=role.id, menu_id=settings_menu.id))

                # 分配设置子菜单
                clustering_menu = menu_by_path.get("/app/settings/clustering")
                if clustering_menu:
                    await db.execute(role_menu.insert().values(role_id=role.id, menu_id=clustering_menu.id))
...
async def verify_role_menus() -> None:
    """验证角色菜单权限"""
