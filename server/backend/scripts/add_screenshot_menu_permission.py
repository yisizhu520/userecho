...
async def add_screenshot_permission():
    """为业务角色添加截图识别菜单权限"""
    async with async_db_session.begin() as db:
        ...
        # 1. 查找截图识别菜单
        screenshot_menu = await db.scalar(
            select(Menu).where(Menu.path == '/app/feedback/screenshot')
        )
        ...
        for role_name in roles_to_grant:
            ...
            existing = await db.scalar(
                select(role_menu.c.role_id).where(
                    ...
                    role_menu.c.menu_id == screenshot_menu.id
                )
            )
            ...
            # 添加权限
            await db.execute(
                role_menu.insert().values(
                    role_id=role.id,
                    menu_id=screenshot_menu.id
                )
            )
...
async def verify_permissions():
    ...
    async with async_db_session() as db:
        ...
        result = await db.execute(
            select(Role).join(
                role_menu, role_menu.c.role_id == Role.id
            ).where(
                role_menu.c.menu_id == screenshot_menu.id
            )
        )
