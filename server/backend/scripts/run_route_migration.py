...
async def verify_migration():
    ...
    async with async_db_session() as db:
        ...
        business_roles = await db.scalars(
        ...
        # 2. 检查系统管理菜单路径
        admin_menus = await db.scalars(
            select(Menu).where(Menu.path.like('/admin/%'))
        )
...
def main():
    ...
    # Step 3: 验证迁移
    if success:
        if not asyncio.run(verify_migration()):
            print('\n⚠️  迁移验证发现问题')
            success = False
