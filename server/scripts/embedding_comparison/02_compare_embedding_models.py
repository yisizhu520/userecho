...
def evaluate_random_baseline(annotations: list[dict]) -> dict:
    ...
    # 打印结果
    print("模型: Random Baseline")
    print(f"Precision: {precision:.3f}")
    print(f"Recall: {recall:.3f}")
    print(f"F1 Score: {f1:.3f}")
    print(f"AUC: {auc:.3f}")
    print("\n⚠️  随机基线用于证明实验有效性")
    print("   如果最好模型的 F1 Score 接近随机基线,说明模型几乎没用\n")
...
def generate_comparison_report(results: list[dict], output_file: Path) -> None:
    ...
    if successful_results:
        ...
        report_lines.append(
            f"   - 相似度分离度: {best_model['similarity_distribution']['similar_mean'] - best_model['similarity_distribution']['dissimilar_mean']:.3f}"
        )
        report_lines.append("")
        ...
        if baseline and best_model["model"] != "volcengine":
            f1_improvement = (
                (best_model["metrics"]["f1_score"] - baseline["metrics"]["f1_score"])
                / baseline["metrics"]["f1_score"]
                * 100
            )
            suspicious_reduction = (
                (baseline["suspicious_pairs_count"] - best_model["suspicious_pairs_count"])
                / max(baseline["suspicious_pairs_count"], 1)
                * 100
            )

            report_lines.append("📊 **相比 Volcengine 基线**:")
