...
async def cluster_with_dbscan(feedbacks_data: list[dict], embeddings: list[list[float]]) -> dict:
    ...
    """
    print(f"\n{'='*80}")
    print(f"方案0：DBSCAN（旧方案，固定阈值）".center(80))
    print(f"{'='*80}\n")
    ...
    return _build_result("DBSCAN", feedbacks_data, embeddings, clusters, {
        "algorithm": "dbscan",
        "threshold": threshold,
...
async def cluster_with_hdbscan(feedbacks_data: list[dict], embeddings: list[list[float]]) -> dict:
    ...
    """
    print(f"\n{'='*80}")
    print(f"方案1：HDBSCAN（自适应阈值）".center(80))
    print(f"{'='*80}\n")
    ...
    return _build_result("HDBSCAN", feedbacks_data, embeddings, clusters, {
        "algorithm": "hdbscan",
        "min_cluster_size": min_cluster_size,
        "min_samples": min_samples,
    })
...
async def cluster_with_hdbscan_llm(feedbacks_data: list[dict], embeddings: list[list[float]]) -> dict:
    ...
    """
    print(f"\n{'='*80}")
    print(f"方案2：HDBSCAN + LLM 语义校验".center(80))
    print(f"{'='*80}\n")
    ...
    for cluster_indices in initial_clusters:
        if len(cluster_indices) >= llm_min_size:
            ...
            if result.is_valid:
                ...
                # LLM 建议拆分为子聚类
                split_count += 1
                print(f"   🔄 聚类 {validation_count}: {len(cluster_indices)} 条反馈 - 拆分为 {len(result.sub_clusters)} 个子聚类")
                print(f"      原因: {result.reason}")
                
                # 根据 LLM 建议创建子聚类
                for sub_cluster in result.sub_clusters:
                    # feedback_indices 是从 1 开始的，需要转换为实际索引
                    sub_indices = [cluster_indices[i-1] for i in sub_cluster.feedback_indices if 0 < i <= len(cluster_indices)]
    ...
    return _build_result("HDBSCAN+LLM", feedbacks_data, embeddings, refined_clusters, {
...
def _build_result(method: str, feedbacks_data: list[dict], embeddings: list[list[float]], 
                  clusters: list[list[int]], params: dict[str, Any]) -> dict:
    """
    构建聚类结果
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        for key, result in results.items():
            ...
            for cluster in top_clusters:
                f.write(f"#### 聚类 #{cluster['cluster_id']} ({cluster['size']} 条, 相似度: {cluster['avg_similarity']:.3f})\n\n")
        ...
        f.write(f"- **HDBSCAN vs HDBSCAN+LLM**: ")
