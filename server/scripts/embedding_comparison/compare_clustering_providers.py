...
async def cluster_feedbacks(feedbacks_data: list[dict], provider: str) -> dict:
    ...
    print(f"📈 相似度分布:")
    print(f"   min={upper_triangle.min():.3f}, max={upper_triangle.max():.3f}, "
          f"mean={upper_triangle.mean():.3f}, median={np.median(upper_triangle):.3f}")
    print(f"   > 0.90: {(upper_triangle > 0.90).sum()}/{len(upper_triangle)} "
          f"({100 * (upper_triangle > 0.90).sum() / len(upper_triangle):.1f}%)")
    print(f"   > 0.80: {(upper_triangle > 0.80).sum()}/{len(upper_triangle)} "
          f"({100 * (upper_triangle > 0.80).sum() / len(upper_triangle):.1f}%)")
    print(f"   > 0.70: {(upper_triangle > 0.70).sum()}/{len(upper_triangle)} "
...
async def main():
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        for provider, result in results.items():
            ...
            f.write(f"| **{provider.upper()}** | {result['cluster_count']} | {result['clustered_count']} | {result['noise_count']} | {clustering_rate:.1f}% |\n")
        ...
        for provider, result in results.items():
            ...
            for cluster in top_clusters:
                f.write(f"### 聚类 #{cluster['cluster_id']} ({cluster['size']} 条反馈, 平均相似度: {cluster['avg_similarity']:.3f})\n\n")
    ...
    for provider, result in results.items():
        clustering_rate = result['clustered_count'] / result['total_feedbacks'] * 100
        print(f"{provider.upper():15} | 聚类数: {result['cluster_count']:3} | 聚类率: {clustering_rate:5.1f}% | 噪声: {result['noise_count']:3}")
