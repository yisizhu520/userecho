...
async def check_pg_trgm_extension() -> bool:
    """检查 pg_trgm 扩展是否启用"""
...
async def check_gin_indexes() -> bool:
    """检查 GIN 索引是否创建"""
    ...
    async with async_engine.begin() as conn:
        ...
        expected_indexes = {"idx_feedbacks_content_gin", "idx_feedbacks_ai_summary_gin"}
        found_indexes = set()
...
async def test_search_performance():
    ...
    test_cases = [
        ...
        (
            "搜索 ai_summary 字段",
            "SELECT * FROM feedbacks WHERE ai_summary ILIKE '%登录%' AND deleted_at IS NULL LIMIT 10",
        ),
        (
            "搜索 content OR ai_summary",
            "SELECT * FROM feedbacks WHERE (content ILIKE '%登录%' OR ai_summary ILIKE '%登录%') AND deleted_at IS NULL LIMIT 10",
        ),
    ]
...
async def verify_index_usage() -> bool:
    """验证索引是否被使用"""
...
async def print_summary(results: dict) -> None:
    """打印总结"""
    print_section("✨ 验证总结")
    ...
    if all_passed:
        ...
        print("   - 关键词搜索性能已优化")
        print("   - 前端搜索响应时间应 < 100ms")
        print("   - 支持中文、英文模糊搜索")
    else:
        print(f"{Colors.RED}{Colors.BOLD}⚠️  部分检查未通过{Colors.NC}\n")
        print(f"{Colors.YELLOW}📖 解决方案：{Colors.NC}")
        print("   1. 确保数据库是 PostgreSQL")
        print("   2. 运行迁移：alembic upgrade head")
        print("   3. 查看文档：docs/development/fulltext-search-index-guide.md")
