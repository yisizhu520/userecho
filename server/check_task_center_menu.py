...
async def check_task_center_menu():
    ...
    async with async_db_session() as db:
        ...
        if task_menu.parent_id:
            parent = await db.scalar(select(Menu).where(Menu.id == task_menu.parent_id))
            if parent:
                print("父菜单信息：")
        ...
        if task_menu.parent_id:
            ...
            siblings = await db.scalars(select(Menu).where(Menu.parent_id == task_menu.parent_id).order_by(Menu.sort))
            for sibling in siblings:
                status_icon = "✅" if sibling.status == 1 else "❌"
                display_icon = "👁️" if sibling.display == 1 else "🙈"
                current = "👉" if sibling.id == task_menu.id else "  "
                print(
                    f"  {current} {status_icon}{display_icon} [{sibling.sort:2d}] {sibling.title} (path={sibling.path})"
                )
