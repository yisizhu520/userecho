...
async def test_sidebar_logic():
    ...
    async with async_db_session() as db:
        ...
        # 3. 查找系统管理员角色
        system_admin_role = await db.scalar(select(Role).where(Role.name == "系统管理员"))
        ...
        system_menus = await db.scalars(
            select(Menu).where(Menu.status == 1, Menu.display == 1, Menu.path.like("/system%")).order_by(Menu.sort)
        )
