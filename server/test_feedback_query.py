...
async def test_feedback_query() -> None:
    """测试反馈查询"""
    ...
    async with async_db_session() as db:
        ...
        # 2. 统计各状态的反馈数
        status_query = (
            select(Feedback.clustering_status, func.count(Feedback.id).label("count"))
            .where(
                Feedback.tenant_id == tenant_id,
                Feedback.deleted_at.is_(None),
            )
        ...
        for fb in samples:
            log.info(
                f"  - ID: {fb.id[:8]}... | topic_id: {fb.topic_id or 'NULL'} | status: {fb.clustering_status} | content: {fb.content[:50]}..."
            )
