...
async def add_task_center_menu():
    ...
    async with async_db_session() as db:
        ...
        if not settings_menu:
            ...
            # 列出所有顶级菜单
            print("\n当前顶级菜单：")
            top_menus = await db.scalars(select(Menu).where(Menu.parent_id is None).order_by(Menu.sort))
        ...
        # 检查"任务中心"是否已存在
        existing = await db.scalar(select(Menu).where(Menu.name == "TaskCenter"))
        ...
        max_sort = await db.scalar(
            select(text("COALESCE(MAX(sort), 0)")).select_from(Menu.__table__).where(Menu.parent_id == parent_id)
        )
