...
async def check_clustering_quality():
    async with async_db_session() as db:
        ...
        print("\n质量门槛：")
        print(
            f"  要求 silhouette >= {min_silhouette}: {'✅' if silhouette >= min_silhouette else '❌'} (当前: {silhouette:.4f})"
        )
        print(
            f"  要求 noise_ratio <= {max_noise_ratio}: {'✅' if noise_ratio <= max_noise_ratio else '❌'} (当前: {noise_ratio:.4f})"
        )
